...
def scrape_ALSC(path):

    start = time.time()
    print('-'*75)
    print('Scraping alsc-awards-shelf.org ...')
    print('-'*75)
    ...
    if path == '':
        ...
        links = []
        nbooks, npages = 0, 0
        homepage = 'https://alsc-awards-shelf.org/directory/results?page='
        while True:
            npages += 1
            url = homepage + str(npages)
            driver.get(url)
            # scraping books urls
            titles = wait(driver, 5).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "h2.title")))
            for title in titles:
                try:
                    nbooks += 1
                    ...
                    print('The below error occurred during the scraping from ALSC.com, retrying ..')
                    print('-'*50)
                    print(err)
                    continue
        ...
        with open('ALSC_links.csv', 'w', newline='\n', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    if path != '':
        ...
        df_links = pd.read_csv('ALSC_links.csv')
    ...
    for i, link in enumerate(links):
        try:
            ...
            des = ''
            try:
                div = wait(driver, 2).until(EC.presence_of_element_located((By.XPATH, "//div[@id='product-secondary']")))
            ...
            try:
                ...
                for tag in tags:
                    text = tag.get_attribute('textContent')
                    if 'By' in text:
                        author = text.replace('By', '').replace('\n', '').strip()
                        authors_links = wait(tag, 2).until(EC.presence_of_all_elements_located((By.TAG_NAME, "a")))
            ...
            # other info
            genre, cat, age, form, ISBN = '', '', '', '', ''
            try:
                info = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "ul.details.mute")))
                ...
                for li in lis:
                    ...
                    if 'Age:' in text:
                        ...
                        form = text.replace('Format:', '').strip()      
                        continue
                    elif 'ISBN:' in text:
                        ISBN = text.replace('ISBN:', '').strip()
                        continue
                    try:
                        url = wait(li, 2).until(EC.presence_of_element_located((By.TAG_NAME, "a"))).get_attribute('href')
                        if 'genre' in url:
                            genre = text.replace('\n', '').strip()
                        elif 'booklist' in url:
                            cat = text.replace('\n', '').strip()
                    except:
                        pass                    
            except:
    ...
    print('-'*75)
    print(f'ALSC.com scraping process completed successfully! Elapsed time {elapsed} mins')
    print('-'*75)
    driver.quit()
